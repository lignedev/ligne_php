...
class PhpFormBuilder {
	...
	/**
	 * Contruye el HTML segun los aprametros ya asignados
	 *
	 * @param bool $echo se puede pasar opcionalmente true
     * para imprimir por pantalla inmediatamente, de no pasarle este parametro
     * solo asignara su valor a una variable
	 *
	 * @return string
	 */
	function build_form( $echo = false ) {
        ...
        **/
		...
		foreach ( $this->inputs as $val ) :
			...
			if ( ! empty( $label_html ) ) {
				...
				if ( $val['required'] ) {
				    //Si el formulario es requerido se le agrega un aterisco
